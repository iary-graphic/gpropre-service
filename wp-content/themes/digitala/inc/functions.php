...
function digitala_page_builder_components($components)
{
...
function digitala_default_components($components)
{
...
function digitala_register_components($components = array())
{
    if (apply_filters('colibri_page_builder/installed', false)) {
        $components = digitala_page_builder_components($components);
    } else {
        $components = digitala_default_components($components);
    }

    return $components;
}

Hooks::prefixed_add_action('components', 'digitala_register_components');
Theme::load(array(
    'themeRelativePath' 		=> '',
 ...
 */
function digitala_assets()
{
    return digitala_theme()->getAssetsManager();
}
...
add_action('after_setup_theme', function() {
    digitala_theme()->register_menus(array(
        'header-menu' => esc_html__('Header Menu', 'digitala'),
        'footer-menu' => esc_html__('Footer Menu', 'digitala'),
    ));
...
add_action('widgets_init', 'digitala_register_sidebars');
function digitala_register_sidebars()
{
    register_sidebar(array(
        'name' 			=> esc_html__('Blog sidebar widget area', 'digitala'),
    ...
    if (function_exists('\is_shop')) {
        register_sidebar(array(
            'name' 			=> esc_html__('Woo Commerce left sidebar widget area', 'digitala'),
            'id' 			=> 'digitala-ecommerce-left',
            'before_widget' => '<div id="%1$s" class="widget %2$s">',
            'before_title' 	=> '<h5 class="widgettitle">',
            'after_title' 	=> '</h5>',
            'after_widget' 	=> '</div>',
    ...
    if (!apply_filters('colibri_page_builder/installed', false)) {
        digitala_assets()
            ...
            ->registerStylesheet("digitala-theme", "/theme/theme.css")
    ...
    digitala_assets()->registerTemplateStyle('digitala-theme-extras', '/theme/extras.css', 'digitala-theme');
...
//blog options
    ...
    function digitala_show_post_meta_setting_filter($value)
    {
    ...
    add_filter('colibri_show_post_meta', 'digitala_show_post_meta_setting_filter');


    function digitala_posts_per_row_setting_filter($value)
    {
    ...
    add_filter('colibri_posts_per_row', 'digitala_posts_per_row_setting_filter');

    function digitala_archive_post_highlight_setting_filter($value)
    {
    ...
    add_filter('colibri_archive_post_highlight', 'digitala_archive_post_highlight_setting_filter');


    function digitala_blog_sidebar_enabled_setting_filter($value)
    {
    ...
    Hooks::prefixed_add_filter('blog_sidebar_enabled', 'digitala_blog_sidebar_enabled_setting_filter');
//add_filter( 'blog_sidebar_enabled', 'digitala_blog_sidebar_enabled_setting_filter' );

    function digitala_override_with_thumbnail_image($value)
    {
        global $post;
    ...
    add_filter('colibri_override_with_thumbnail_image', 'digitala_override_with_thumbnail_image');

    function digitala_print_archive_entry_class($class = "")
    {

        $classes = array("post-list-item", "h-col-xs-12 h-col-md-12 h-col-lg-6", "space-bottom");
        $classes = array_merge($classes, explode(" ", $class));
        $classes = get_post_class($classes);

        $default = get_theme_mod('blog_posts_per_row', Defaults::get('blog_posts_per_row'));
        $postsPerRow = max(1, apply_filters('digitala_posts_per_row', $default));


        $classes[] = "h-col-sm-12 h-col-md-" . (12 / intval($postsPerRow));

        $classes = apply_filters('digitala_archive_entry_class', $classes);
    ...
    function digitala_print_masonry_col_class($echo = false)
    {

        global $wp_query;
        $index = $wp_query->current_post;
        $hasBigClass = (is_sticky() || ($index === 0 && apply_filters('digitala_archive_post_highlight', false)));
    ...
    Hooks::prefixed_add_filter('info_page_tabs', 'digitala_get_started_info_page_tab');

    function digitala_get_started_info_page_tab($tabs)
    {
    ...
    function digitala_theme_plugins($plugins)
    {
        $theme_plugins = array();
    ...
    Hooks::prefixed_add_filter('theme_plugins', 'digitala_theme_plugins');


    add_filter('http_request_host_is_external', 'digitala_allow_internal_host', 10, 3);
    function digitala_allow_internal_host($allow, $host, $url)
    {
    ...
    add_action('wp_ajax_digitala_front_set_predesign', function () {
        check_ajax_referer( 'digitala_front_set_predesign_nonce', 'nonce' );
    ...
    function digitala_override_main_row_class($classes)
    {
        return Defaults::get('templates.blog.row.layout-classes', $classes);
    }

    Hooks::prefixed_add_filter('main_row_class', 'digitala_override_main_row_class', 10, 1);
...
function digitala_has_fresh_site() {
	$mods = get_theme_mods();
