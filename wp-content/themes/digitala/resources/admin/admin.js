...
// admin notice
(function ($) {
    var $notice_container = $('.digitala-admin-big-notice--container');
    ...
    function disableNotice() {
        wp.ajax.post('digitala_disable_big_notice', {
            nonce: digitala_builder_status.digitala_disable_big_notice_nonce
        });
    }

    function toggleProcessing(value) {
        $(window).on('beforeunload.digitala-admin-big-notice', function () {
            return true;
        });
        if (value) {
            $('.digitala-admin-big-notice').addClass('processing');
            $('.digitala-admin-big-notice .action-buttons').fadeOut();
        } else {
            $('.digitala-admin-big-notice').removeClass('processing');
        }
    }
    ...
    function installBuilder(callback) {
        pluginNotice(digitala_builder_status.messages.installing);
        $.get(digitala_builder_status.install_url).always(function () {
            toggleProcessing(true);
            activateBuilder(callback);
        }).always(function () {
            $(window).off('beforeunload.digitala-admin-big-notice');
        });
    }

    function activateBuilder(callback) {
        pluginNotice(digitala_builder_status.messages.activating);

        wp.ajax.post('digitala_activate_plugin', {
            slug: digitala_builder_status.slug,
            source: 'notice-' + selectedFrontPage,
            _wpnonce: digitala_builder_status.colibri_plugin_install_activate_nonce
        }).done(function () {
            $(window).off('beforeunload.digitala-admin-big-notice');
            if (callback) {
                callback();
    ...
    function processBuilderInstalationStepts(callback) {
        wp.ajax.post('digitala_front_set_predesign', {
            index: selectedFrontPage,
            nonce: digitala_builder_status.digitala_front_set_predesign_nonce
        });
        if (digitala_builder_status.status === "not-installed") {
            toggleProcessing(true);
            installBuilder(callback);
        }

        if (digitala_builder_status.status === "installed") {
            toggleProcessing(true);
            activateBuilder(callback);
        }
    }
    ...
    $notice_container.on('click', '.view-all-demos', function () {
        ...
        processBuilderInstalationStepts(function () {
            window.location = digitala_builder_status.view_demos_url;
        });
    });
    ...
    var digitalaInstallPluginSuccess = function digitalaInstallPluginSuccess(response) {
    ...
    var digitalaInstallPlugin = function digitalaInstallPlugin(event) {
        ...
        wp.updates.installPlugin({
            ...
            success: digitalaInstallPluginSuccess
        });
    };

    var digitalaActivatePlugin = function digitalaActivatePlugin(event) {
        var $button = $(event.target);
    ...
    $document.on("click", ".colibri-plugin.install-now", digitalaInstallPlugin);
    $document.on("click", ".colibri-plugin.activate-now", digitalaActivatePlugin);
